...
|---|---|---|---|---|---|---|
| `searchable` | Both | iOS 15 / macOS 12 / watchOS 8 / tvOS 15 / visionOS 1 | Current | Implemented | `Sources/SwiftOpenUI/Modifiers/SearchableModifier.swift` | Batch D fallback on GTK/Win32/Web: search field above content; placement stored but not differentiated yet; tokens and editableTokens render as display-only chips; suggestions render as simple rows with click-to-complete behavior; scopes render as simple mutually exclusive controls. Search UI is hidden when isPresented == false. |
